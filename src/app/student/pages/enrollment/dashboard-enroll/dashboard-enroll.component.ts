...
export class DashboardEnrollComponent implements OnInit {
  ...
  @ViewChild('confirmPresent') confirmPresent: any;
  dataCicle:any;
  dataStudent:any;
  crossdata: any;
  savedSchedule:any;
  ...
  getCourses(){
    // this.enrollmentS.getDebt(this.session.getItem('emplidSelected'))
    //   .then((res)=> {
  ...
  changeSchedule(course, evt, alreadyIn = false){
    ...
    if(course.notAvailable){
      ...
      if(course.UCS_REST_DET2MREU[0]['MODALIDAD'] != 'Virtual'){
        ...
        if (el.ASOCIACION_CLASE == course.ASOCIACION_CLASE) {
          ...
          if (el.value) {
            if (this.checkCrosses(el)) {
              ...
              if(this.checkInPerson(course, alreadyIn)){
                this.savedSchedule = {course: course, evt: evt};
                this.confirmPresent.open();
              }
            }
          }
        } else {
  ...
  checkInPerson(crs, on?){
    if (this.myCoursesinEnrollment && !on) {
      for (let i = 0; i < this.myCoursesinEnrollment.length; i++) {
        if (this.myCoursesinEnrollment[i].CICLO_LECTIVO == crs.CICLO_LECTIVO) {
          if(crs.UCS_REST_DET2MREU){
            for (var o = 0; o < crs.UCS_REST_DET2MREU.length; o++) {
              for (var u = 0; u < this.myCoursesinEnrollment[i].UCS_REST_MTG_DET_REQ.length; u++) {
                if (!this.myCoursesinEnrollment[i].UCS_REST_MTG_DET_REQ[u]['DESCR_INSTALACION'].includes('VIRT') && crs.UCS_REST_DET2MREU[o]['TIPO'] != 'VIRT') {
                  if (BetweenDays(this.myCoursesinEnrollment[i].UCS_REST_MTG_DET_REQ[u]['INICIO_FECHA'] + ' 00:00:00',this.myCoursesinEnrollment[i].UCS_REST_MTG_DET_REQ[u]['FIN_FECHA'] + ' 00:00:00', RealDate(new Date(crs.UCS_REST_DET2MREU[o]['FECHA_INICIAL'].replaceAll('-', '/') + ' 00:00:00'))) || BetweenDays(this.myCoursesinEnrollment[i].UCS_REST_MTG_DET_REQ[u]['INICIO_FECHA'] + ' 00:00:00',this.myCoursesinEnrollment[i].UCS_REST_MTG_DET_REQ[u]['FIN_FECHA'] + ' 00:00:00', RealDate(new Date(crs.UCS_REST_DET2MREU[o]['FECHA_FINAL'].replaceAll('-', '/') + ' 00:00:00')))) {
                    if (this.getDayY(this.myCoursesinEnrollment[i].UCS_REST_MTG_DET_REQ[u]) == crs.UCS_REST_DET2MREU[o]['DIA'].normalize("NFD").replace(/[\u0300-\u036f]/g, "").toUpperCase()) {
                      if(this.myCoursesinEnrollment[i].UCS_REST_MTG_DET_REQ[u].MODALIDAD != crs.UCS_REST_DET2MREU[o]['MODALIDAD']){
                        return true
                      }
                    }
                  }
  ...
  continue(con){
    if(con){
      this.confirmPresent.close();
    } else {
      this.savedSchedule.course.value = false;
      this.savedSchedule.evt.target.checked = false;
      for (let j = 0; j < this.scheduleAvailables.length; j++) {
        if (this.scheduleAvailables[j].ASOCIACION_CLASE == this.savedSchedule.course.ASOCIACION_CLASE) {
          this.scheduleAvailables[j].value = false;
        }
      }
      this.confirmPresent.close();
    }
  ...
  confirmEnroll(){
    ...
    let data = [];
    for (var i = 0; i < this.scheduleAvailables.length; i++) {
      if (this.scheduleAvailables[i].value && this.scheduleAvailables[i]['UCS_REST_DET2MREU']) {
        for (var o = 0; o < this.scheduleAvailables[i]['UCS_REST_DET2MREU'].length; o++) {
          data.push({
            INSTITUTION: this.dataStudent['INSTITUTION'],
            ACAD_CAREER: this.dataStudent['ACAD_CAREER'],
  ...
  checkDuplicates(array){
    ...
    array.sort(this.dynamicSortMultiple(["-SECCION","-NRO_CLASE","-CODIGO_COMPONENTE"]));
